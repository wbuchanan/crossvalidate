...
real scalar mcaccuracy(string scalar pred, string scalar obs, string scalar touse) {
	...
	// Return the accuracy
	return(sum(diagonal(conf))/sum(conf))
...
// Defines multiclass Kappa -- similar to accuracy, but normalized by accuracy expected 
//	by random chance
...
// based on: https://github.com/tidymodels/yardstick/blob/main/R/class-mcc.R
real scalar mcmccc(string scalar pred, string scalar obs, string scalar touse) {
